...
class Inventory(Entity):
    ...
    def __init__(self):
        ...
        self.main_inventory = Entity(
            parent=camera.ui, model="quad", texture="textures/hud/inventory/static.png"
        )
        ...
        item_holder_scale = (
            self.quick_inv.scale_x - item_holder_distance * 2
        ) / self.MINI_GRID_X
        self.quick_inv_items = [
            Entity(
                ...
                position=Vec3(
                    self.quick_inv.position.x
                    - self.quick_inv.scale_x / 2
                    + item_holder_distance
                    + item_holder_scale * (i + 0.5),
                    self.quick_inv.position.y,
                    z=-0.1,
                ),
            )
    ...
    def update(self):
        ...
        for item in self.quick_inv_items:
            item.enabled = True
