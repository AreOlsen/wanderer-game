...
class Background(Entity):
    def __init__(self, z_position, movement_factor):
        super().__init__()
        ...
        self.image_sprites = [(
            Entity(
                ...
                ppu=16,
                parent=camera,
                world_position=Vec3(camera.position.x, camera.position.y, z_position-i/1000),
            ),
            Entity(
                texture=path,
                model="quad",
    ...
    def update(self):
        ...
        for j, image_set in enumerate(self.image_sprites):
            offset_x = -(camera.position.x/(self.s_x*self.movement_factor**j))%1
            for i, image in enumerate(image_set, start = 0):
                image.position = Vec3(offset_x*self.s_x-self.s_x*(i),image.position.y,image.position.z)